...
@export var platform_count: int = 5000
@export var platform_width_range: Vector2i = Vector2i(100, 800) # Min and Max width of platforms
@export var platform_height: int = 70 # Height of the platform
@export var map_size: Vector2i = Vector2i(15000, 600) # Map size
@export var vertical_probability: float = 0.3 # Probability that a platform is vertical
...
func _process(delta):
	pass
	# Spawn circles
	#if randi() % 30 == 0: # spawn every 30 seconds
...
func generate_platforms():
	...
	for i in platform_count:
		...
		while not valid_position and attempts < max_attempts:
			...
			if (x_position + platform_width) > 100:# Ensure there is a gap vertically
				valid_position = true
